...
void updatePlayer() {
    ...
    if ((~((*(volatile unsigned short *)0x04000130)) & ((1 << 5))) && (player.worldCol + 8) > 24 && !leftCollision()) {
        player.worldCol -= player.cdel;
    }


    if ((~((*(volatile unsigned short *)0x04000130)) & ((1 << 4))) && (player.worldCol + player.width + 8) < 216 && !rightCollision()) {
        player.worldCol += player.cdel;
    }
    ...
    if ((!(~(oldButtons) & ((1 << 6))) && (~buttons & ((1 << 6)))) && groundCollision()) {
        player.rdel -= 1500;
        player.worldRow += player.rdel;
    }
...
void animatePlayer() {
    ...
    if ((~((*(volatile unsigned short *)0x04000130)) & ((1 << 5))) && (player.worldCol + 5) > 24 && !leftCollision()) {
        player.aniState = SPRITELEFT;
    }
...
int groundCollision() {
    ...
    if (level1RoughCollisionMapBitmap[((player.screenRow + player.height - 1 + (vOff % 512)) * (256) + (player.screenCol + (8) + (hOff % 256)))] == 0) {
        return 1;
    }

    if (level1RoughCollisionMapBitmap[((player.screenRow + player.height - 1 + (vOff % 512)) * (256) + (player.screenCol + (8) + (hOff % 256) + player.width))] == 0) {
        return 1;
    }
    return 0;
}
...
int level1WinCollision() {
    ...
    if (level1RoughCollisionMapBitmap[((player.screenRow + player.height - 1 + (vOff % 512)) * (256) + (player.screenCol + (8) + (hOff % 256)))] == 0x03E0) {
        return 1;
    }

    if (level1RoughCollisionMapBitmap[((player.screenRow + player.height - 1 + (vOff % 512)) * (256) + (player.screenCol + (8) + (hOff % 256) + player.width))] == 0x03E0) {
        return 1;
    }
    return 0;
}
...
int leftCollision() {
    ...
    for (int i = 0; i < player.height - 3; i++) {
        if (level1RoughCollisionMapBitmap[((player.screenRow + i + (vOff % 512)) * (256) + (player.screenCol + (hOff % 256) + (7)))] == 0) {
            return 1;
        }
    }
    return 0;
...
int rightCollision() {
    ...
    for (int i = 0; i < player.height - 3; i++) {
        if (level1RoughCollisionMapBitmap[((player.screenRow + i + (vOff % 512)) * (256) + (player.screenCol + player.width + (hOff % 256) + (8)))] == 0) {
            return 1;
        }
    }
    return 0;
